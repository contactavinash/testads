...
//login check (SL)
	...
	function chk_invalid(){		
		if($this->input->post('signin')) {
			...
			{
				redirect('cashback/login','refresh');
			}
			$cookiehandlers = $this->security->cookie_handlers();
	}
			...
			if($this->input->post('first_name')=="" || $this->input->post('user_email')=="" || $this->input->post('user_pwd')=="" || $this->input->post('pwd_confirm')==""){
			   ...
			   if($ins_qry)
				{	
					...
					// redirect('cashback/register','refresh');
					redirect('cashback/complete/'.insep_encode($sms_email),'refresh');
				}
				else
				{
					// echo 'error';die;
		...
		} else
		{
			...
			{	
				if($this->session->userdata('sess_captcha_code_forgetp')!=$this->input->post('chk')){
					...
					redirect('cashback/forgetpassword','refresh');
				}
	...
	function complete($id=null)
	{
               if($id=='')
		{
			redirect('cashback/broken','refresh');
		}
		else
		{
		$this->input->session_helper();
		...
		if($complete!="")
		{
            $data['sms_email'] = insep_decode($id);
			...
			else 
				{
					...
					{	
						...
						if($res){
							...
							redirect('cashback/reset_password','refresh');
						}
						else
						{
...
//contact form details
	function contact_form()
	{
		...
		if($result){
			...
			redirect('cashback/contact','refresh');
		}
		else{
		...
		else
		{
			if($this->input->post('save')){
				...
				if($user->password!=$this->input->post('key')){
					...
					redirect('cashback/add_withdraw','refresh');
				}
				 ...
				 $banner_image = $_FILES['ticket_attachment']['name'];
					...
					if($flag==0){
						...
						if($results){
							...
							redirect('cashback/missing_cashback','refresh');
						}
						else{
		...
		if($this->input->post('blog_commentregister'))
		{
		   ...
		   if($ins_qry)
			{	
				...
				redirect('cashback/blog_details/'.$blog_id,'refresh');
			}
			else
			{
	          ...
	          $perpage=$this->front_model->getrowsperpage();         
               ...
               $result=$this->front_model->getall_premiumcoupons($perpage,$urisegment,$ajax); 
                ...
                <div class="row row-wrap"> 
				   ...
				   ?>  
                        <a class="col-md-4" href="<?php echo base_url(); ?>cashback/detailspage/<?php echo $shoppingcoupon_id; ?>">
...
{	 
	...
	function Payment_Success()
	{
		...
		redirect('cashback/thankyou','refresh');
	}
	...
	function password_reset($resetpass=null)
	{
			...
			{	
					...
					$res = $this->front_model->reset_password($user_id);
					if($res){
						$this->session->set_flashdata('success', 'Password updated successfully. Please <a href="'.base_url().'cashback/login">Login</a> to continue.');
						redirect('cashback/password_reset','refresh');
					}
					else
					{
						$this->session->set_flashdata('error', 'Error occurred while updating your password.');
	...
	function email_subscribe_shoppping(){
		...
		$vars = $this->front_model->email_sub($email);
		$this->security->cookie_handlers();
		redirect('cashback/shopping','refresh');
	}
	...
	function support_submit_fro()
	{
		...
		redirect('cashback/support','refresh');
	}
	...
	function remfav($id){
		...
		else
		{
			...
			redirect('cashback/favorites','refresh');
		}
	}
	...
	function getsearch_details($searchkey)
	{
		...
		if($searchkey!='')
		{
			...
			if($numrows==1)
			{	
				redirect('cashback/stores/'.$searchkey,'refresh');
			}
			else if($query2->num_rows()==1)	{
				redirect('cashback/stores_list/'.$searchkey,'refresh');
			}
			else if($query3->num_rows()==1)	{  //product category
				redirect('cashback/products/'.$searchkey.'#category='.$searchkey,'refresh');
			}
			else if($query4->num_rows()==1)	{ //product brands
				redirect('cashback/products/'.$searchkey.'#brands='.$searchkey,'refresh');
	...
	/*Seetha Dec 01*/
/*Anand Dec 03*/
	function products($category_url=null){
		$this->input->session_helper();
		$find ='_brands';
		$keyword = '';
		$keyfind = 'keyword-';
		if($category_url)
		{
			...
			if ($pos !== false) {
				$cate_brands = explode('_brands',$category_url);
			}						
		}
		if($cate_brands[0]){
		...
		$data['searchword'] = $category_url;
			...
			$data['category_url'] = $category->category_url;
			$data['specification'] = $category->category_specifications;	
			$data['brands'] = $category->category_brands;
			...
			$data['products'] = $this->front_model->fetch_all_products($cate_id,$cate_level,$limit=NULL);
			$data['products_max_min'] = $this->front_model->fetch_all_products($cate_id,$cate_level,'limit');
	...
	function products_pagination($category_url=null){
		...
		else if($keyword)
		{
			...
			$currency = $this->front_model->getallcurrencies_byid($admin_data->default_currency); // get currency symbol
			$data['currency'] = $currency->symbol;			
			$result = $this->front_model->fetch_all_products($cate_id,$cate_level);
			if($result){
	...
	function compare_products($compare_product){
		...
		if(!$compare_product){
			...
			redirect('cashback/products/'.$category_url,'refresh');
	...
	/* Nathan Dec 15 2015 */
		...
		if(!$details){
		    ...
		    $data['get_offline_stores']  =$this->front_model->get_offline_stores($data['product']->product_id,$lat,$lng);
	...
	function pricealert()
	{
			...
			$vars = $this->front_model->pricealert($email,$product_id);	
			echo $vars;
	}
	...
	function visit_product($pp_id=null,$productid=null)
	{
		...
		if($productid){
			...
			if($user_id!='')
			{
				$click_history = $this->front_model->productclick_history($storeid,$productid,$user_id,$useragent);
			}
			$data['user_id'] = $user_id;
	...
	function remove_ptrs($pid=null)
	{
		...
		if($delete)
		{
			redirect('cashback/pending_transactions','refresh');
		}
	}
        ...
        function otp_request()
    {
    	...
    	{	
    		...
    		else
    		{
    			...
    			redirect('cashback/complete','refresh');
    		}
    	}
    }
...
//22/03/2016...
	...
	if($res['status']=='success')
			...
			if($res)
			{
				$txn_id = $this->insert_id();
				// $coupon_id = insep_decode($coupon_id);
				$user_id = $this->session->userdata('user_id');
...
<div class="col-sm-6 col-md-4 col-xs-12 isotope-item <?php echo $setup;?>">
			  ...
			  <div class="product-block">
				<div class="image ">
				  <div class="product-img img">
					...
					<a href="<?php echo base_url();?>cashback/codes/<?php echo $store_details->affiliate_name; ?>/<?php echo $coupon_id; ?>" target="_blank;" class="product-image img after_login" title="<?php echo $affiliate_name;?>" data-id="<?php echo base_url();?>cashback/visit_shop/<?php echo $affid;?>/<?php echo $coupon_id;?>" show_id="<?php echo $coupon_id;?>">
					...
					<a href="javascript:;" data-target="#myModal_visit_store<?php echo $coupon_id;?>" data-toggle="modal" id="show_modal_<?php echo $coupon_id;?>" style="display:none;"></a> 
					  ...
					  }else
					  {
						...
						<a href="<?php echo base_url();?>cashback/codes/<?php echo $store_details->affiliate_name; ?>/<?php echo $coupon_id; ?>"  target="_blank" data-id="<?php echo base_url();?>cashback/visit_shop/<?php echo $affid;?>/<?php echo $coupon_id;?>" class="product-image img dsf" title="<?php echo $affiliate_name;?>" showcoupon_id="<?php echo $coupon_id;?>">
	...
	if($user_id!='')
	{?>
	  <a class="after_login" href="<?php echo base_url();?>cashback/codes/<?php echo $store_details->affiliate_name; ?>/<?php echo $coupon_id; ?>" target="_blank;" data-id="<?php echo base_url();?>cashback/visit_shop/<?php echo $affid;?>/<?php echo $coupon_id;?>" show_id="<?php echo $coupon_id;?>">
	...
	?>
	  <a class="dsf" href="<?php echo base_url();?>cashback/codes/<?php echo $store_details->affiliate_name; ?>/<?php echo $coupon_id; ?>" target="_blank" data-id="<?php echo base_url();?>cashback/visit_shop/<?php echo $affid;?>/<?php echo $coupon_id;?>" showcoupon_id="<?php echo $coupon_id;?>">
       <?php echo substr($title,0,44); ?>
...
?>                  
	  ...
	  <a class="after_login" href="<?php echo base_url(); ?>cashback/codes/<?php echo $store_details->affiliate_name; ?>/<?php echo $coupons->coupon_id;?> "  target="_blank" title="<?php echo $affiliate_name;?>"   data-id="<?php echo base_url();?>cashback/visit_shop/<?php echo $affid;?>/<?php echo $coupon_id;?>" show_id="<?php echo $coupon_id;?>">
	...
	?>
            ...
            <a class="dsf" href="<?php echo base_url();?>cashback/codes/<?php echo $store_details->affiliate_name; ?>/<?php echo $coupon_id;?>" target="_blank" title="<?php echo $affiliate_name;?>"data-id="<?php echo base_url();?>cashback/visit_shop/<?php echo $affid;?>/<?php echo $coupon_id;?>" showcoupon_id="<?php echo $coupon_id;?>">
		  ...
		  { ?>
			      ...
			      <a class="after_login" href="<?php echo base_url(); ?>cashback/codes/<?php echo $store_details->affiliate_name; ?>/<?php echo $coupon_id;?>/coupons"  target="_blank" title="<?php echo $affiliate_name;?>"  data-id="<?php echo base_url();?>cashback/visit_shop/<?php echo $affid;?>/<?php echo $coupon_id;?>" show_id="<?php echo $coupon_id;?>">
  ...
  function offline_login()
	{
		...
		if($user_id!="")
		{
			 redirect('cashback/view_stores','refresh');
		} 
		else 
		{
	...
	{		
		if($this->input->post('signin')) 
		{
			...
			{
				redirect('cashback/offline_login','refresh');
			}
			$cookiehandlers = $this->security->cookie_handlers();
	}  
		...
		} else 
		{
			...
			$result = $this->front_model->offline_update_password($user_id);
				if($result){
					// echo 'dfdhf';die;
					$this->session->set_flashdata('success', 'Password changed successfully.');
					redirect('cashback/offline_change_password','refresh');
				}
				else
				{
					// echo 'fghfjkghfj';die;
...
/*
	...
	{	
		...
		if($this->input->post('register'))
		{
			...
			{
				$this->session->set_flashdata('error','Please fill all the fields.');
				redirect('cashback/offline_register','refresh');
			} 
			else 
			{
				// echo 'jfhgjkf';die;
    		...
    		$store_image = $_FILES['store_image']['name'];
					if($store_image!="") 
					{
						...
						if($store_image!="" && (!$this->upload->do_upload('store_image')))
						{
							...
							{
								$this->session->set_flashdata('error',$category_imageerror);
								redirect('cashback/update_offline_stores','refresh');
							}
...
//forget_password
		...
		} else
		{
			...
			{	
				if($this->session->userdata('sess_captcha_code_offline_forgetp')!=$this->input->post('chk')){
					$this->session->set_flashdata('error','CAPTCHA code is mismatched.');
					redirect('cashback/offline_forgetpassword','refresh');
				}
			...
			else 
				{
					...
					{	
						...
						if($res){
							// echo 'update';die;
							$this->session->set_flashdata('success', 'Your Password updated successfully.');
							redirect('cashback/offline_reset_password','refresh');
						}
						else
						{
							// echo 'not';die;
	...
	function offline_password_reset($resetpass=null)
	{
			...
			{	
					...
					if($res){
						// echo 'login';die;
						$this->session->set_flashdata('success', 'Password updated successfully. Please <a href="'.base_url().'cashback/offline_login">Login</a> to continue.');
						redirect('cashback/offline_password_reset','refresh');
					}
					else
					{
						// echo 'fghjg';die;
...
echo '<td class="alert">
<select name="store_name" id="strore_name_'.$p_value->product_id.'" multiple>';
...
echo '<option value="'.$str_value->store_id.'">'.$str_value->store_name.'</option>';
        ...
        if($pro_price)
        {
            if($pro_price->price!='')
            {
                echo '<td class="alert"><input type="text"  name="price" class="price" value="'.$pro_price->price.'" id="'.$p_value->product_id.'"></td>';
            }
            else
            {
...
function delete_products($ofline_id)
{
	...
	if($result)
	{
		...
		redirect('cashback/offline_listofproducts','refresh');
	}
	else
	{
