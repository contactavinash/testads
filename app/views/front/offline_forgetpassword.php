...
<title>Offline Forget Password | <?php $admindetails = $this->front_model->getadmindetails_main(); echo $admindetails->site_name; ?></title>
...
<?php
        ...
        <?php
				...
				$attributes = array('role'=>'form','name'=>'forget_password','id'=>'forget_password','method'=>'post');
					echo form_open('cashback/offline_forgetpassword/',$attributes);
					
		?>
  <div class="form-group form-list">
  <label class="required" for="email_address"><em>*</em>Email Address</label>
    <input type="text" name="email" alt="email" id="email" class="input-text required-entry validate-email form-control" value="<?php echo $email; ?>" />
  </div>
  ...
  <div class="form-group form-list clear">
   ...
   <label for="email_address" class="required"><em>*</em>Security Code</label>
			 <div class="col-md-4">
					<?php
							$a = rand(1000,9999);
							$this->session->set_userdata('sess_captcha_code_offline_forgetp',$a);
							$vals = array(
								'word' => $a,
								'img_width' => '110',
								'img_height' => 36
