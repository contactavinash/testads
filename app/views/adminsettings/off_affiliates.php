...
<body class="fixed-top">
  ...
  <br>
                        ...
                        <form id="form2" action="" method="post" name="form2">
							...
							$k=0;
							if($off_affiliates){
							foreach($off_affiliates as $affiliate){
							$k++;
							...
							?>
                                <tr class="odd gradeX" style="" >
                                    <td style=""><?php echo $k; ?></td>
                                      <td><input type="checkbox"  class="check_b" name="chkbox[<?php echo $affiliate->store_id;?>]" /></td>
                                    <td>
                                    <a href="<?php echo base_url();?>adminsettings/view_stores/<?php echo $affiliate->store_id; ?>"><?php echo $affiliate->store_name; ?></a>
                                    <!--<img src="<?php echo base_url();?>assets/img/web_scraping_icon.png" style="float:right; width: 41px;">--></td>
                                   
                                    
                                   
                                    <td <?php echo $extraset;?>><center><img src="<?php echo base_url();?>uploads/offline_stores/<?php echo $affiliate->store_logo; ?>" width="110" height="35" /></center></td>
                                     
									<td>
                                      <img src="<?php echo base_url();?>assets/img/loadaing.gif" id="import_loader<?php echo $affiliate->store_id; ?>" style="display:none;">
                                   <p id="sts<?php echo $affiliate->store_id; ?>">
									<?php
                                    ...
                                    <td ><?php echo $affiliate->date_added; ?> </td>
									<td id="id_sts<?php echo $affiliate->store_id; ?>">
									<button  type="button" onclick="return change_offline_status('<?php echo $affiliate->store_id; ?>','<?php echo $status ?>');"><i class="icon-exchange"></i></button>
									
									</td>
                                    <td class="center hidden-phone">
									<?php
									$confirm = array("class"=>"confirm-dialog","onclick"=>"return confirmDelete('Do you want to delete this affiliate detail?');");		
									echo anchor('adminsettings/delete_offaffiliate/'.$affiliate->store_id,'<i class="icon-trash"></i>',$confirm); ?>
									</td>
                                </tr>
 ...
 function change_offline_status(id,status)
 {
	 ...
	 $.ajax({
			  type: 'POST',
			   ...
			   url: '<?php echo base_url();?>adminsettings/change_offline_status', 
			   success: function(data){
				   if(data==1)
				   {
					...
					if(status==1)
					{
						...
						$('#id_sts'+id).html('<button  type="button" onclick="return change_offline_status('+id+',0);"><i class="icon-exchange"></i></button>');
					}
					else
					{
						$('#sts'+id).show();
						$('#sts'+id).html('Active');
						$('#id_sts'+id).html('<button  type="button" onclick="return change_offline_status('+id+',1);"><i class="icon-exchange"></i></button>');
					}
				   }
				  }
