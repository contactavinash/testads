...
if(!file_exists("paytm/".$shop."_".$category."_".$subCategory)){
	mkdir("paytm/".$shop."_".$category."_".$subCategory);
}
echo "PAGE = ".$i."\n";
...
foreach($items["grid_layout"] as $item){
	...
	if(!file_exists("paytm/".$shop."_".$category."_".$subCategory."/".$item["product_id"].".json")){
		RETRY:
		$data=curl_exec($c);
		if(empty($data)){
			goto RETRY;
		}
		file_put_contents("paytm/".$shop."_".$category."_".$subCategory."/".$item["product_id"].".json",$data);
		echo "\nADDED = ".$item["name"];
	}
}
?>
